...
```bash
awk [options] 'criteria {action}' <path/to/file>
```

## If-Else
```bash
awk '{ if (condition) {action} else {action} }' <path/to/file>
```

## BEGIN and END blocks
```bash
awk 'BEGIN {} {action} END {}' input file
```
* `BEGIN {}` executes commands before processing any input
* `END {}` executes commands after all input processed
...
/^>/ {
    if (seq) {
        if (length(seq) >= cutoff) print header "\n" seq;
    }
    header = $0; seq = "";
    next
}
...
## Convert wrapped-sequence format FASTA into single-line format
```bash
