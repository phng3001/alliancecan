...
#SBATCH --job-name=run_apptainer_ncbi-genome-download
...
$container ncbi-genome-download all \
--section genbank \
...
if [[ -n "$SLURM_JOB_ID" && "$SLURM_JOB_ID" -ne 0 ]]; then
    ...
    if [[ -f "slurm-${SLURM_JOB_ID}.out" ]]; then
        mv slurm-${SLURM_JOB_ID}.out run_apptainer_ncbi-genome-download-${SLURM_JOB_ID}.out
    fi
fi
