...
#SBATCH --job-name=__SAMPLE___eggnog

######### Preprocessing #########

# Check if the correct number of arguments is provided
if [ "$#" -ne 2 ]; then
    echo "Usage on terminal: bash $0 <eggnog_container> <file_extension>"
    echo "Usage on cluster: sbatch $0 <eggnog_container> <file_extension>"
    exit 1
fi
...
# Default option picked from http://eggnog-mapper.embl.de/ for Genomic data using Prodigal as Gene prediction method 
apptainer exec \
-W $TMPDIR \
$eggnog_container emapper.py \
-i __SAMPLE__.${file_extension} \
-o __SAMPLE__ \
...
if [[ -n "$SLURM_JOB_ID" && "$SLURM_JOB_ID" -ne 0 ]]; then
    ...
    if [[ -f "slurm-${SLURM_JOB_ID}.out" ]]; then
        mv slurm-${SLURM_JOB_ID}.out __SAMPLE___eggnog-${SLURM_JOB_ID}.out
    fi
fi