...
#SBATCH --job-name=run_apptainer_pyseer_pres_lmm
...
if [ $# -lt $MANDATORY_ARGS ]; then
    ...
    echo "Usage on terminal: bash $0 <container> <pyseer_script_dir> <genotype_matrix> <phenotype_file> <phylogenetic_tree> [alpha] [prefix]"
    echo "Usage on cluster: sbatch $0 <container> <pyseer_script_dir> <genotype_matrix> <phenotype_file> <phylogenetic_tree> [alpha] [prefix]"
    exit 1
fi
...
if [[ -n "$SLURM_JOB_ID" && "$SLURM_JOB_ID" -ne 0 ]]; then
    ...
    if [[ -f "slurm-${SLURM_JOB_ID}.out" ]]; then
        mv slurm-${SLURM_JOB_ID}.out run_apptainer_pyseer_pres_lmm-${SLURM_JOB_ID}.out
    fi
fi
