...
if [ "$#" -ne 5 ]; then
    echo "Usage on terminal: bash $0 <container> <reference> <sample_name> <single-end_reads> <minfrac>"
    echo "Usage on cluster: sbatch $0 <container> <reference> <sample_name> <single-end_reads> <minfrac>"
    exit 1
fi
