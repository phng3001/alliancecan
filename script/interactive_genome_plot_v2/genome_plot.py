...
def detect_gff_gtf(file_path):
    ...
    print(
        f"Cannot detect GFF3 or GTF format in {file_path}. Exiting.",
        file=sys.stderr
    )
    sys.exit(1)
        ...
        for j in range(i + 1, len(intervals_sorted)):
            ...
            if chrom1 == chrom2 and s2 <= e1:
                ...
                id1 = (
                    ...
                    or attrs1.get("ID")
                    or f"{chrom1}:{start1}-{end1}"
                )

                id2 = (
                    attrs2.get("locus_tag")
                    or attrs2.get("ID")
                    or f"{chrom2}:{start2}-{end2}"
                )
...
def main():
    ...
    for chrom in chrom_order:
        ...
        for f in chrom_feature_map.get(
            chrom,
            []
        ):
